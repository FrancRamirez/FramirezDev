// js/pago.js
//
// Maneja los botones de compra ("Comprar") de la sección Aplicaciones:
//   0) Al cargar la página, pide /api/pagos/pago (GET) para mostrar el
//      precio en el botón y, si hay sesión y la cuenta ya compró la app,
//      cambiar el botón a "Descargar" directo (sin pasar por el modal).
...
(function () {
  ...
  // la cuenta logueada ya compró esa app, convertirlo en un botón de
  // descarga directa (verde, sin pasar por el modal de pago).
  async function inicializarBotones() {
    ...
    try {
      ...
      botones.forEach((btn) => {
        if (btn.dataset.appId !== config.appId) return;

        const precioEl = btn.querySelector('[data-precio-label]');
        if (config.yaComprado) {
          ...
          btn.setAttribute('data-download-owned-app', config.appId);
        } else if (precioEl) {
          precioEl.textContent = formateadorPrecio.format(config.precio);
        }
      });
    } catch (error) {
